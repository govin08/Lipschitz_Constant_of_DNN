...
\usepackage{epsfig,longtable,verbatim,graphicx,multirow,array,wasysym,esint,hyperref,kotex,color,algorithm2e}
...
&=\sum_{i=1}^{k-1}\frac{\partial\theta_k}{\partial\theta_i}\frac{\partial\theta_i}{\partial\boldsymbol x},
\end{align*}
it follows that
\begin{align*}
...
&\le\sum_{i=1}^{k-1}
\max\left|\left|\frac{\partial\theta_k}{\partial\theta_i}\right|\right|\cdot L_i.
\end{align*}
...
\SetAlgoLined
\KwData{the function \(f=g_K\circ\cdots\circ g_1\),\\ where \(\theta_k=(g_k\circ\cdots\circ g_1)(\boldsymbol x)\) for \(1\le k\le K\)}
\KwResult{An upper bound \(L\) for \(\lip(f)\)}
\(k\gets 1\)\;
\(L_1\gets1\)\;
\While{\(k<K\)}{
\(L_k\gets\sum_{i=1}^{k-1}\left(\max\left|\left|\frac{\partial\theta_k}{\partial\theta_i}\right|\right|\right)L_i\)\;
\(k\gets k+1\)\;
}
\(L\gets L_K\)
\caption{AutoLip}
...
%%%
\subsection{AutoLip applied to 2-layered-perceptron}

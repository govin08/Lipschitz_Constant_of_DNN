...
%The Lipschitz constant of the simplest function, the linear map, corresponds to the operator norm or the matrix.
%To study the Lipschitz constant of the simplest function or the linear map, one should encounter the concept of the operator norm of a matrix, or the absolute value of the dominating eigenvalue of the matrix.
...
After \(K\) iterations, an upper bound \(L_K\) of the lipschitz constant for \(\theta_K=f(\boldsymbol x)\) is obtained.
(Algorithm \ref{alg:1})

\begin{algorithm}
\KwData{the function \(f=g_K\circ\cdots\circ g_0\),\\ where \(\theta_k=(g_k\circ\cdots\circ g_0)(\boldsymbol x)\) for \(0\le k\le K\)}
...
+\sup\norm{\frac{\partial\theta_2}{\partial\theta_1}}\times L_1=%||W||\times1+||\bx||\times0
||W||\\
k=3;\:\: L_3&
...
% and we can use the Theorem \ref{theo:evaluation_for_rectangular_matrices}.
...
\begin{algorithm}
\KwData{A rectangular matrix \(F\in\mathbb R^{n\times m}\)}
\KwResult{Approximation for \(||F||\)}
\(A\gets F^TF\)\;
...
\bibitem {SS-FS} Singla, S., \& Feizi, S. (2019). Robustness certificates against adversarial examples for relu networks. arXiv preprint arXiv:1902.01235.

\bibitem {LR} Larson, R. (2016). \emph{Elementary linear algebra}. Cengage Learning.
\end{thebibliography}

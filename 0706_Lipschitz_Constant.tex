...
\begin{equation}\label{eq:square_matrix_inequality}
\tilde\lambda\le||W||.
\end{equation}
...
Still, we have, \(\lambda_1=-2\), \(\lambda_2=3\) and \(||W_4||\ge3\) by \eqref{eq:square_matrix_inequality}.
...
\]
Since \(W_5\) has the only eigenvector \(0\), the inequality \eqref{eq:square_matrix_inequality} still holds.
...
Now, we turn our attention to the problem of finding the optimal Lischitz constant of various architectures frequently appearing in machine learning and deep learning.
...
But, it is not always easy or feasible to find the true value of the Lipschitz constant whenever the function \(f\) is given.
Indeed, even for simple algorithm such as two-layered MLP, it takes beyond polynomial time to evaluate the optimal constant.
...
The Lipschitz condition we impose is not the global one \eqref{eq:LC1} ; it only need to be locally Lipschitz in a sense that the bounding constant \(c\) exists for each \(\bx\).
...
Here, \(D_{\bx}f\) is the Jacobian matrix of \(f\) or a matrix \(A\) satisfying
\[\lim_{\by\to\bx}\frac{||f(\by)-f(\bx)-A(\by-\bx)||}{||\by-\bx||}=0.\]
And \(||D_{\bx}(f)||\) is nothing but the operator norm we've defined in the previous chapter.

The first statement of the theorem --- the differentiability --- is usually refered to the main conclusion of this theorem.
But the proof of it is so lengthy that we omit the proof here.
As for one-dimensional case, many of the textbooks of measure theory including \cite{WR} covered this issue.
In the process, concepts like absolute continuity, Radon-Nykodym theorem, total variation are involved.
A proof for multi-dimensional case is found, for example, in \cite{VC}.

The second statement, or the optimal Lipschitz constant is the one that we can use for our purpose.
We think of the matrix whose entries are partial derivatives at point \(\bx\), compute the matrix norm and take the supremum over all \(\bx\in\mathbb R^n\).
But, it doesn't mean that we can always find the accurate value \(\lip(f)\), given \(f\) and \(\bx\).
To evaluate \(\lip(f)\), we should find the supremum, or the maximum if it exists.
The theorem converts the problem of evaluating the constant to the problem of optimization problem.
...
\bibitem {KC} Keith Conrad. {\em Computing the norm of a matrix}. \url{https://kconrad.math.uconn.edu/blurbs/linmultialg/matrixnorm.pdf}
...
\bibitem {VC} Villani, Cédric. {\em Optimal transport: old and new.} Vol. 338. Berlin: springer, 2009.
